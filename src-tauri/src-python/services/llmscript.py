...
def get_wikipedia_facts(location_name: str, lat: float, lng: float, lang: str = "en") -> str:
    ...
    def fetch_from_wiki(search_lang: str) -> str:
        try:
            ...
            if results:
                ...
                req_geo = urllib.request.Request(geo_url, headers={'User-Agent': 'NaviviApp/1.0'})
            ...
            req_ext = urllib.request.Request(ext_url, headers={'User-Agent': 'NaviviApp/1.0'})
            with urllib.request.urlopen(req_ext) as res_ext:
                ...
                for pid, pdata in pages.items():
                    ...
                    extract = pdata.get('extract', '')[:250] # Keep it short to save tokens
...
def sanitize_location_name(name: str) -> str:
    ...
    if re.match(r'^\d+$', clean) or len(clean) < 2:
        return ""
    invalid_placeholders = ["locating...", "unknown location", "waypoint", "null", "undefined"]
    if clean.lower() in invalid_placeholders:
        return ""
    return clean

def generate_voiceover_script(prompt: str, location_name: str, lat: float, lng: float, engine: str = "ollama") -> str:
    ...
    if has_user_prompt and not valid_name:
        ...
        subject_header = f"■ 目的地のテーマ・メモ: {prompt}\n■ 座標: ({lat:.4f}, {lng:.4f})"
    ...
    if lang == "ja":
        ...
        """
        user_msg = "このスポットの魅力や移動の楽しさを伝えるナレーションを作成してください。"
    ...
    try:
        ...
        if engine == "ollama":
            ...
            payload = json.dumps({"system_instruction": {"parts": [{"text": system_instruction}]}, "contents": [{"parts": [{"text": user_msg}]}]}).encode("utf-8")
            req = urllib.request.Request(url, data=payload, headers={'Content-Type': 'application/json'})
            with urllib.request.urlopen(req) as res:
                data = json.loads(res.read().decode('utf-8'))
                clean_text = re.sub(r'（.*?）|\(.*?\)', '', data['candidates'][0]['content']['parts'][0]['text'])
                return clean_text.strip()

        elif engine == "groq":
            api_key = os.environ.get("GROQ_API_KEY", "")
            url = "https://api.groq.com/openai/v1/chat/completions"
            payload = json.dumps({"model": "llama3-8b-8192", "messages": messages}).encode("utf-8")
            req = urllib.request.Request(url, data=payload, headers={'Content-Type': 'application/json', 'Authorization': f'Bearer {api_key}'})
            with urllib.request.urlopen(req) as res:
                data = json.loads(res.read().decode('utf-8'))
                clean_text = re.sub(r'（.*?）|\(.*?\)', '', data['choices'][0]['message']['content'])
                return clean_text.strip()
...
def generate_overview_script(waypoints: list[str], engine: str = "ollama") -> str:
        if not waypoints:
            return "素晴らしい旅の始まりです。さあ、出発しましょう！" if engine == "ollama" else "Welcome to our journey. Let's get started!"
        ...
        try:
            ...
            if engine == "ollama":
                ...
                with urllib.request.urlopen(req) as res:
                    data = json.loads(res.read().decode('utf-8'))
                    return re.sub(r'（.*?）|\(.*?\)', '', data['candidates'][0]['content']['parts'][0]['text']).strip()
                ...
                with urllib.request.urlopen(req) as res:
                    data = json.loads(res.read().decode('utf-8'))
                    return re.sub(r'（.*?）|\(.*?\)', '', data['choices'][0]['message']['content']).strip()
