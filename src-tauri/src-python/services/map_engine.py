...
# [Config] Constants for map generation
TARGET_ASPECT_RATIO: Final[float] = 16 / 9
MIN_MAP_WIDTH_PX: Final[int] = 1280


# [Core] MapEngine Class
class RouteGeometry:
    """Handles spatial mathematics, index building, smoothing, and projections."""

    # [Map] Compute bounding box with padding for a DataFrame of lat/lon points
    @staticmethod
    def get_bounding_box(
        df: pd.DataFrame, padding_factor: float = 0.05, **kwargs
    ) -> Dict[str, float]:
    ...
    @staticmethod
    def douglas_peucker(
        points: List[Tuple[float, float]], tolerance: float, **kwargs
    ) -> List[int]:
        if len(points) < 3:
    ...
    # [Map] Generate a smooth path between waypoints
    @staticmethod
    def get_smooth_path(
        ...
        **kwargs,
        ...
        if len(filtered_pts) > 2:
            keep_idx = RouteGeometry.douglas_peucker(
...
# [Util] RoutePacing Class
class RoutePacing:
    """Calculates temporal durations based on physical geographic distances."""

    # [Animation] Compute segment durations based on waypoints and route DataFrame
    @staticmethod
    def compute_segment_durations(
        ...
        # Import GPSMath from our newly split converter file
        from services.gps_converter import GPSMath
        ...
        for i in range(n_segments):
            ...
            if len(chunk) > 1:
                ...
                # Call haversine_vectorized via the GPSMath class
                seg_distances.append(
                    float(
                        np.nansum(GPSMath.haversine_vectorized(lat1, lon1, lat2, lon2))
                    )
                )
            else:
    ...
    # [Animation] Compute chunk durations based on sequence data
    @staticmethod
...
# [Map] TileDownloader Class
class TileDownloader:
    """Manages contextily maps, cache directories, zoom logic, and image cropping."""

    # [Config] Initialize the TileDownloader with job configuration and map provider
    ...
    # [Map] Fetch an overview image for a bounding box with specified output size and zoom
    def fetch_overview_image(
        ...
        # Fail Fast: If coordinates are missing, stop immediately
        if s is None or n is None or e is None or w is None:
            raise ValueError("Invalid bounding box: missing coordinate values.")

        # Calculate spans and center safely (only happens once now)
        center_lat = (s + n) / 2.0
        ...
        while optimal_zoom > 0:
            try:
                img, extent = cx.bounds2img(
                    ...
                    use_cache=str(self.cache_dir),  # type: ignore
    ...
    def fetch_residential_chunk(
        ...
        start_lat, start_lon = float(chunk_df["latitude"].iloc[0]), float(
            chunk_df["longitude"].iloc[0]
        )
        end_lat, end_lon = float(chunk_df["latitude"].iloc[-1]), float(
            chunk_df["longitude"].iloc[-1]
        )
        ...
        optimal_zoom = (
            ...
            if span_meters <= 300
                ...
                if span_meters <= 600
                    ...
                    17 if span_meters <= 1200 else (16 if span_meters <= 2500 else 15)
                )
            )
        )
