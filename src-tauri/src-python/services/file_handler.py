...
# [CORE] RAW GPS FILE MANAGER
class RawGPSFileManager:
    """Manages secure storage and collision-free naming of incoming raw GPS files."""

    # [Config/IO] Default storage directory for raw GPS files
    def __init__(
        self, storage_dir: Union[str, Path] = Path("data/inputs/gpsdata/rawdata")
    ):
        self.storage_dir = Path(storage_dir)

    # [IO] Store raw GPS file with timestamped and sequenced filename
    def store_file(self, source_path_str: Union[str, Path]) -> str:
        ...
        if not source_path.exists():
            logger.error(f"Error: Could not find raw file at {source_path}")
            return ""
        ...
        for i in range(1, 100):
            ...
            logger.error("Error: Exceeded 99 files in the exact same second!")
            return ""

        try:
            logger.info(f"Storing raw file safely as {new_filename}...")
...
# [CORE] PROJECT ASSET HANDLER
class ProjectAssetHandler:
    """Handles project-specific asset storage and cross-platform normalization."""

    # [IO] Save an uploaded image into the project's 'assets' directory with a unique timestamped filename
    @staticmethod
    def save_asset_image(
        project_dir: Union[str, Path], source_image_path: Union[str, Path]
    ) -> str:
        """
        ...
        logger.info(f"Asset image saved to: {target_path}")

        return str(target_path).replace("\\", "/")


# [CORE] TTS AUDIO GENERATOR
class TTSAudioGenerator:
    """Interfaces with the local text-to-speech service to generate audio files."""

    # [Config] Default TTS server URL
    def __init__(self, server_url: str = "http://localhost:8000/v1/audio/speech"):
        self.server_url = server_url

    # [TTS/IO] Generate TTS audio from text and save it to a specified output path
    def generate_and_save(
        self, text: str, output_path: Union[str, Path] = "output.mp3"
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            logger.error(
                "Error: Could not connect to the Irodori-TTS server. Make sure it is running on http://localhost:8000"
            )
            return None
        except Exception as e:
...
# [CORE] PROJECT INITIALIZER
class ProjectInitializer:
    """Manages workspace folder layouts and initializes project configuration states."""

    # [Config/IO] Base directory for all user projects
    def __init__(self, base_dir: Union[str, Path] = Path("data/projects")):
        self.base_dir = Path(base_dir)

    # [IO] Initialize a new project folder structure and create a project.json metadata file
    def initialize(
        self,
        user_id: str,
        project_name: str,
        ...
        logger.info(f"Project initialized successfully at: {config_path}")
        return str(config_path)


# [CONFIG] BACKWARDS-COMPATIBLE MODULE-LEVEL FUNCTIONS (FACADE WRAPPERS)
...
def save_project_asset_image(
    project_dir: str | Path, source_image_path: str | Path
) -> str:
...
def generate_and_save_audio(
    ...
    server_url: str = "http://localhost:8000/v1/audio/speech",
) -> str | None:
    generator = (
        TTSAudioGenerator(server_url=server_url)
        if server_url != "http://localhost:8000/v1/audio/speech"
        else _tts_generator
    )
...
def initialize_new_project(
    user_id: str, project_name: str, base_settings: Optional[dict] = None
) -> str:
    return _project_initializer.initialize(user_id, project_name, base_settings)


# [CORE] FILEHANDLER FACADE CLASS FOR EXTERNAL USAGE
class FileHandler:
    @staticmethod
    def store_raw_file_with_datetime(source_path_str: str) -> str:
