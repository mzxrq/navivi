...
import {
  ...
  MousePointer2, Scissors, Sparkles, Magnet,
...
export function TimelineView() {
      ...
      if (marquee) {
        ...
        if (mRight - mLeft > 5 || mBottom - mTop > 5) {
          const selected = timeline.clips.filter(clip => {
            ...
            return !(cRight < mLeft || cLeft > mRight || tb.bottom < mTop || tb.top > mBottom);
          });
          if (selected.length > 0) setSelectedClipIds(selected.map(c => c.id));
        }
        setMarquee(null);
      }
    };
  ...
  const handleZoom = (newZoom: number) => setTimeline({ ...timeline, zoomMultiplier: newZoom });

  const handleSplitClip = (clipId: string, splitTime: number) => {
    const clip = timeline.clips.find(c => c.id === clipId);
    const track = timeline.tracks.find(t => t.id === clip?.trackId);
    if (!clip || track?.isLocked || splitTime <= clip.startTime || splitTime >= clip.startTime + clip.duration) return;

    const clip1 = { ...clip, duration: splitTime - clip.startTime };
    const clip2 = { ...clip, id: crypto.randomUUID(), startTime: splitTime, duration: clip.duration - clip1.duration, sourceOffset: (clip.sourceOffset || 0) + clip1.duration };
    setTimeline({ ...timeline, clips: [...timeline.clips.filter(c => c.id !== clipId), clip1, clip2] });
  };

  const handleSelectClip = (id: string | null, multi: boolean = false) => {
    if (!id) { setSelectedClipIds([]); return; }
    if (multi) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        case " ":
          e.preventDefault(); setIsPlaying((prev) => !prev); break;
          case "v":
          if (isCtrl) {
            e.preventDefault();
            ...
            // We will mutate these arrays and apply them to the state at the end
            let newTracks = [...timeline.tracks];
            let newClips = [...timeline.clips];
            const pastedClipIds: string[] = [];

            // Group the copied clips by the track they originally came from
            const clipsByTrack = copiedClips.reduce((acc, clip) => {
            ...
            Object.entries(clipsByTrack).forEach(([originalTrackId, clips]) => {
              const originalTrack = timeline.tracks.find(t => t.id === originalTrackId);
              if (!originalTrack) return;

              // Check if pasting these clips causes a collision on their original track
              const hasCollision = clips.some(pastedClip => {
                const newStart = currentTime + (pastedClip.startTime - minStart);
                ...
                return timeline.clips.some(existing => {
                   ...
                   // Collision happens if the times overlap (ignoring exact edge touches)
              ...
              // ✨ THE MAGIC: If it collides, dynamically spawn a new track!
              if (hasCollision) {
                 targetTrackId = crypto.randomUUID();
                 const isVideo = originalTrack.type !== 'audio';
                 const prefix = isVideo ? "VIDEO" : "AUDIO";
                 
                 // Count existing tracks to name the new one (e.g., "VIDEO 3")
                 const existingCount = newTracks.filter(t => t.name.toUpperCase().includes(prefix)).length;
              ...
              // Drop the clips into the target track
              clips.forEach(c => {
            ...
            // Apply everything to the timeline at once
            setTimeline({
              ...timeline,
              tracks: newTracks,
              clips: newClips
        ...
        case "c":
          if (isCtrl) {
            e.preventDefault();
            const toCopy = timeline.clips.filter(c => selectedClipIds.includes(c.id));
            if (toCopy.length > 0) {
          ...
          break;
        case "s":
          if (selectedClipIds.length > 0) selectedClipIds.forEach(id => handleSplitClip(id, currentTime));
          break;
          case "backspace":
            case "delete":
              if (selectedClipIds.length > 0) {
                const lockedTracks = timeline.tracks.filter(t => t.isLocked).map(t => t.id);
                const toDelete = selectedClipIds.filter(id => {
                  // ✨ FIX: Explicitly type 'c' so TypeScript doesn't panic
                  const clip = timeline.clips.find((c: ClipData) => c.id === id);
                  return clip && !lockedTracks.includes(clip.trackId);
                });
                
                // ✨ FIX: Use the 'timeline' object directly instead of the 'prev' callback
                setTimeline({ 
                  ...timeline, 
                  clips: timeline.clips.filter((c: ClipData) => !toDelete.includes(c.id)) 
                });
                setSelectedClipIds([]);
              }
              break;
        case "home":
          e.preventDefault(); setCurrentTime(0); break;
        case "end":
          e.preventDefault(); 
          const dur = timeline.clips.reduce((max, clip) => Math.max(max, clip.startTime + clip.duration), 0);
          setCurrentTime(dur); break;
        case "pageup":
          e.preventDefault();
          const edgesPrev = [0, ...timeline.clips.flatMap(c => [c.startTime, c.startTime + c.duration])];
          const prevEdge = Math.max(...edgesPrev.filter(t => t < currentTime - 0.01)) || 0;
          setCurrentTime(prevEdge); break;
        case "pagedown":
          e.preventDefault();
          const edgesNext = [...timeline.clips.flatMap(c => [c.startTime, c.startTime + c.duration])];
          const nextEdge = Math.min(...edgesNext.filter(t => t > currentTime + 0.01));
          if (nextEdge !== Infinity) setCurrentTime(nextEdge); break;
      }
    };
    ...
    const playLoop = (time: number) => {
      if (isPlaying) {
        ...
        setCurrentTime((prevTime) => {
          const totalDur = timeline.clips.reduce((max, clip) => Math.max(max, clip.startTime + clip.duration), 0);
          const nextTime = prevTime + deltaTime;
          if (nextTime >= totalDur && totalDur > 0) { setIsPlaying(false); return totalDur; }
          return nextTime;
        });
      }
      lastTime = time;
  ...
  const handleUpdateTrackName = (trackId: string, newName: string) => {
    ...
    setTimeline({ ...timeline, tracks: timeline.tracks.map((t) => t.id === trackId ? { ...t, name: newName.trim() } : t) });
  };

  const handleToggleTrackProp = (trackId: string, prop: "isHidden" | "isMuted" | "isLocked") => {
    setTimeline({ ...timeline, tracks: timeline.tracks.map(t => t.id === trackId ? { ...t, [prop]: !t[prop] } : t) });
  };

  const handleUpdateClip = (id: string, updates: any) => {
    setTimeline({ ...timeline, clips: timeline.clips.map((c) => c.id === id ? { ...c, ...updates } : c) });
  };

  const PRELOAD_SECONDS = 0.5;
  const activeVisualClips = timeline.clips.filter((clip) => {
    const track = timeline.tracks.find((t) => t.id === clip.trackId);
    if (!track || track.isHidden) return false; 
    const isVisual = track.name.toLowerCase().includes("video") || track.name.toLowerCase().includes("popup");
    return isVisual && currentTime >= (clip.startTime - PRELOAD_SECONDS) && currentTime <= (clip.startTime + clip.duration);
  });

  useEffect(() => {
    const unlisten = listen("render-complete", async (_event) => {
      if (metadata?.directory_path) await autoLoadTimeline(metadata.directory_path);
    });
  ...
  const handleExportVideo = async () => {
    ...
    const success = await saveTimelineManifest(metadata.directory_path, metadata.project_name || "Project", timeline);
    if (success) invoke("export_video", { projectDir: metadata.directory_path }).catch(console.error);
  };
    ...
    <div className="flex flex-col flex-1 h-full bg-white dark:bg-[#09090b] overflow-hidden select-none">
      <div className="flex-1 flex min-h-0 border-b border-zinc-200 dark:border-white/5">
        ...
        <div className="flex-1 p-4 md:p-6 flex flex-col items-center justify-center bg-zinc-50/50 dark:bg-navidark-800 min-w-0 min-h-0 relative overflow-hidden">
          <div className="w-full max-w-4xl max-h-full flex flex-col items-center justify-center gap-4">
            <div className="w-full aspect-video bg-black border border-zinc-800 relative overflow-hidden group">
        ...
        <div className="w-80 bg-white dark:bg-navidark-800 border-l border-zinc-200 dark:border-navidark-400 flex flex-col shrink-0">
          <div className="flex border-b border-zinc-200 dark:border-navidark-700 bg-zinc-50 dark:bg-navidark-900/50">
            <button onClick={() => setRightPanelTab("inspector")} className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider transition-colors ${rightPanelTab === "inspector" ? "border-b-2 border-navi text-navi" : "text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-200"}`}>Inspector</button>
            <button onClick={() => setRightPanelTab("export")} className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider transition-colors ${rightPanelTab === "export" ? "border-b-2 border-navi text-navi" : "text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-200"}`}>Export</button>
          </div>
          <div className="flex-1 overflow-y-auto custom-scrollbar p-4">
            {rightPanelTab === "inspector" ? <Inspector selectedClipIds={selectedClipIds} onClearSelection={() => setSelectedClipIds([])} /> : <ExportPanel onExport={handleExportVideo} />}
          </div>
        </div>
      </div>
      ...
      <div className="h-11 bg-white dark:bg-navidark-800 border-b border-zinc-200 dark:border-navidark-400 flex items-center justify-between px-4 shrink-0 z-30">
          ...
          <div className="flex items-center gap-1 bg-zinc-100 dark:bg-navidark-900 p-1 rounded-md border border-zinc-200 dark:border-navidark-700">
            <button onClick={() => setCurrentTime(0)} className="p-1.5 text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-200 transition-colors rounded hover:bg-zinc-200 dark:hover:bg-navidark-700" title="Home">
            ...
            <button onClick={() => setCurrentTime(timeline.clips.reduce((max, c) => Math.max(max, c.startTime + c.duration), 0))} className="p-1.5 text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-200 transition-colors rounded hover:bg-zinc-200 dark:hover:bg-navidark-700" title="End">
        ...
        <div className="w-40 flex flex-col shrink-0 border-r border-zinc-300 dark:border-navidark-400 bg-zinc-50 dark:bg-navidark-800 z-40">
            <div className="h-8 w-full border-b border-zinc-300 dark:border-navidark-400 shrink-0 flex items-center px-3 bg-zinc-200/90 dark:bg-navidark-800/90">
              <span className="text-[9px] font-bold text-zinc-400 dark:text-zinc-500 tracking-widest">TRACKS</span>
            </div>
            <div 
              ref={headerRef} 
                  ...
                  return (
                    ...
                    >
                      {editingTrackId === track.id ? (
                        <input type="text" autoFocus defaultValue={track.name} onBlur={(e) => handleUpdateTrackName(track.id, e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") handleUpdateTrackName(track.id, e.currentTarget.value); if (e.key === "Escape") setEditingTrackId(null); }} className="w-full bg-white dark:bg-navidark-900 text-[10px] font-bold text-zinc-900 dark:text-zinc-100 px-1.5 py-1 rounded outline-none border-2 border-navi" />
                      ) : (
                        <span className="text-[10px] font-bold text-zinc-500 dark:text-navidark-125 uppercase tracking-wider truncate mb-1">{track.name}</span>
                      )}
                      <div className="flex items-center gap-2">
                        {!isAudioTrack ? (
                          <button onClick={() => handleToggleTrackProp(track.id, "isHidden")} className={`p-1 rounded transition-colors ${track.isHidden ? "text-red-500 bg-red-50 dark:bg-red-500/10" : "text-zinc-400 hover:text-zinc-800 dark:hover:text-zinc-200"}`}>
                            {track.isHidden ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                          </button>
                        ) : (
                          <button onClick={() => handleToggleTrackProp(track.id, "isMuted")} className={`p-1 rounded transition-colors ${track.isMuted ? "text-red-500 bg-red-50 dark:bg-red-500/10" : "text-zinc-400 hover:text-zinc-800 dark:hover:text-zinc-200"}`}>
                            {track.isMuted ? <VolumeX className="w-3.5 h-3.5" /> : <Volume2 className="w-3.5 h-3.5" />}
                          </button>
                        )}
                        <button onClick={() => handleToggleTrackProp(track.id, "isLocked")} className={`p-1 rounded transition-colors ${track.isLocked ? "text-red-500 bg-red-50 dark:bg-red-500/10" : "text-zinc-400 hover:text-zinc-800 dark:hover:text-zinc-200"}`}>
          ...
          <div className="flex-1 flex flex-col min-w-0 relative bg-zinc-50 dark:bg-navidark-800/50">
            <div ref={rulerRef} className="h-8 w-full border-b border-zinc-300 dark:border-navidark-400 bg-zinc-200/90 dark:bg-navidark-800/90 overflow-hidden shrink-0">
              <div className="h-full cursor-ew-resize relative shrink-0" style={{ width: `${timelinePixelWidth}px` }} onMouseDown={(e) => { if (isPlaying) setIsPlaying(false); setIsScrubbing(true); handleScrub(e.clientX); }}>
                ...
                <div className="absolute bottom-0 -translate-x-1/2 w-3 h-3 bg-red-500 [clip-path:polygon(50%_100%,0_0,100%_0)] z-50 pointer-events-none" style={{ left: `${currentTime * pixelsPerSecond}px` }} />
              </div>
            </div>
            ...
            <div 
              ...
              onMouseDown={(e) => {
                // ✨ MARQUEE START: Only fire if clicking the background grid!
            ...
            >
              <div className="relative min-w-max pb-24" style={{ width: `${timelinePixelWidth}px` }}>
                <div className="absolute top-0 bottom-0 w-[1.5px] bg-red-500 z-35 pointer-events-none shadow-[0_0_10px_rgba(239,68,68,0.5)]" style={{ left: `${currentTime * pixelsPerSecond}px` }} />
