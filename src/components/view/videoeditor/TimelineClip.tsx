...
import { convertFileSrc } from "@tauri-apps/api/core"; // ✨ NEW: Safely load local files
...
export function TimelineClip({
  ...
  if (clip.type === "image" || clip.type === "static_popup")
        ...
        if (isRippleMode) {
          ...
          newClips = newClips.flatMap((c) => {
            ...
            if (
              ...
              return [{ ...c, duration: finalStart - c.startTime, groupId: undefined }]; // ✨ FIX
            }
            if (
              c.startTime >= finalStart &&
        ...
        setTimeline({
          ...
          clips: timeline.clips.map(c => {
            ...
            if (c.groupId === clip.groupId) {
              return { ...c, startTime: Math.max(0, c.startTime + deltaStart) };
            }
            return c;
          })
        });
