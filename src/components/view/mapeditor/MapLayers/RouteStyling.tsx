...
export function RouteStyling() {
  ...
  return (
    <div className="relative flex items-center">
      ...
      {isOpen && (
        <div className="absolute top-12 right-0 w-60 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-xl border border-zinc-200 dark:border-white/10 rounded-2xl shadow-2xl p-5 z-40 animate-in fade-in zoom-in-95">
          ...
          <div className="p-4 space-y-5">
            <div className="flex gap-3">
              <div className="flex-1 space-y-1.5">
                ...
                <div className="relative group">
                  <input
                    type="color"
                    value={rgbToHex(settings.line_color || [0, 200, 255])}
                  ...
                  <div className="w-full h-8 rounded-lg border border-zinc-200 dark:border-white/10 shadow-sm flex items-center px-2 gap-2 bg-zinc-50 dark:bg-zinc-900/50 group-hover:border-zinc-400 dark:group-hover:border-zinc-600 transition-colors">
                    <div className="w-4 h-4 rounded-full border border-black/10" style={{ backgroundColor: rgbToHex(settings.line_color || [0, 200, 255]) }} />
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 space-y-1.5">
                ...
                <div className="relative group">
                  <input
                    type="color"
                    value={rgbToHex(settings.marker_color || [0, 0, 255])}
                    onChange={(e) => updateSettings({ marker_color: hexToRgb(e.target.value) })}
                    className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                  />
                  <div className="w-full h-8 rounded-lg border border-zinc-200 dark:border-white/10 shadow-sm flex items-center px-2 gap-2 bg-zinc-50 dark:bg-zinc-900/50 group-hover:border-zinc-400 dark:group-hover:border-zinc-600 transition-colors">
                    <div className="w-4 h-4 rounded-full border border-black/10" style={{ backgroundColor: rgbToHex(settings.marker_color || [0, 0, 255]) }} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <div className="flex-1 space-y-1.5">
                <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Border</label>
                <div className="relative group">
                  <input
                    type="color"
                    value={rgbToHex(settings.line_border_color || [255, 255, 255])}
                    onChange={(e) => updateSettings({ line_border_color: hexToRgb(e.target.value) })}
                    className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                  />
                  <div className="w-full h-8 rounded-lg border border-zinc-200 dark:border-white/10 shadow-sm flex items-center px-2 gap-2 bg-zinc-50 dark:bg-zinc-900/50 group-hover:border-zinc-400 dark:group-hover:border-zinc-600 transition-colors">
                    <div className="w-4 h-4 rounded-full border border-black/10" style={{ backgroundColor: rgbToHex(settings.line_border_color || [255, 255, 255]) }} />
                  </div>
                </div>
              </div>

              <div className="flex-1 space-y-2 pt-1 flex flex-col justify-end">
                <div className="flex justify-between items-end">
                  <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">B-Width</label>
                  <span className="text-xs font-mono font-medium text-zinc-700 dark:text-zinc-300">{settings.line_border_width || 0}px</span>
                </div>
                <input
                  type="range" min="0" max="12"
                  value={settings.line_border_width || 0}
                  onChange={(e) => updateSettings({ line_border_width: parseInt(e.target.value) })}
