...
class IrodoriTTSClient:
    ...
    def __init__(self, output_dir: Path = Path("data/outputs/audio"), base_url: str = "http://127.0.0.1:8088/v1/audio/speech"):
...
class AudioProcessor:
    ...
    def analyze_pauses(self, wav_path: str, silence_threshold: int = 500, min_pause_duration: float = 0.2) -> Dict[str, Any]:
    ...
    def concatenate_files(self, audio_paths: List[str], final_output_path: str = "outputs/master_narration.wav") -> str:
    ...
    def make_silent_audio(self, duration_seconds: float, output_path: str, sample_rate: int = 44100, channels: int = 2, as_wav: bool = False) -> str:
        ...
        cmd = [
            ...
            "-t", f"{max(duration_seconds, 0.05):.3f}",
            *codec_args,
            output_path,
        ]
        ...
        try:
            for i, (duration, narration_path) in enumerate(zip(segment_durations, segment_narration_audio)):
                if narration_path and os.path.exists(narration_path):
                    ...
                    self.make_silent_audio(duration, silent_path, sample_rate=ref_sample_rate, channels=ref_channels, as_wav=True)
    ...
    @staticmethod
    def _detect_reference_audio_format(narration_paths: List[Optional[str]], default_sample_rate: int = 44100, default_channels: int = 2) -> Tuple[int, int]:
...
class VideoProcessor:
    ...
    def normalize_segment_audio(video_path: str, has_narration: bool, sample_rate: int, channels: int, temp_dir: str = "") -> str:
        ...
        audio_proc.make_silent_audio(duration, silent_aac, sample_rate=sample_rate, channels=channels, as_wav=False)
    ...
    @staticmethod
    def concatenate_segments(video_paths: List[str], final_output_path: str = "outputs/final_navigation_video.mp4") -> str:
    ...
    @staticmethod
    def combine_video_and_audio(video_path: str, audio_path: str, final_output_path: str = "outputs/final_output_with_audio.mp4") -> str:
        """Muxes the master audio track with the fully concatenated video."""
...
class TTSPipelineManager:
    ...
    def analyze_pauses(self, wav_path: str, silence_threshold: int = 500, min_pause_duration: float = 0.2) -> Dict[str, Any]:
        return self.audio_processor.analyze_pauses(wav_path, silence_threshold, min_pause_duration)
    ...
    def assemble_final_deliverable(
        ...
        normalized_paths = [
            self.video_processor.normalize_segment_audio(path, has_narration=has_narr, sample_rate=ref_sample_rate, channels=ref_channels)
...
def analyze_wav_pauses(wav_path: str, silence_threshold: int = 500, min_pause_duration: float = 0.2) -> dict:
    return _default_manager.analyze_pauses(wav_path, silence_threshold, min_pause_duration)
