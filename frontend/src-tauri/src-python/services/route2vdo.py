...
class VideoExporter:
    ...
    def __init__(self, output_path: str, width: int, height: int, fps: int):
        ...
        if self.proc is None:
            ...
            self._fallback_writer = cv2.VideoWriter(
                self._fallback_path, cv2.VideoWriter.fourcc(*"XVID"), self.fps, (self.width, self.height)
            )
    ...
    def release(self, output_path: str) -> str:
        ...
        if output_path.lower().endswith(".mp4") and self._fallback_path and self._reencode_to_h264(self._fallback_path, output_path):
...
class GraphicsEngine:
    ...
    def __init__(self, line_color=(0, 200, 255), line_thickness=10, marker_color=(0, 0, 255), marker_radius=18):
    ...
    def draw_path(self, frame: np.ndarray, path_history: List[Tuple[int, int]]):
        if len(path_history) > 1:
            cv2.polylines(frame, [np.array(path_history, dtype=np.int32)], False, 
                          self.line_color, self.line_thickness, cv2.LINE_AA)

    def draw_marker(self, frame: np.ndarray, cx: int, cy: int):
        cv2.circle(frame, (cx, cy), self.marker_radius, self.marker_color, -1, cv2.LINE_AA)
        cv2.circle(frame, (cx, cy), self.marker_radius + 4, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.circle(frame, (cx, cy), self.marker_radius + 7, self.marker_color, 1, cv2.LINE_AA)
    ...
    def prebake_landmark_sprite(self, label: str) -> Tuple[np.ndarray, Tuple[int, int]]:
        ...
        cv2.circle(sprite, (cx, cy), self.marker_radius, (255, 80, 0, 255), -1, cv2.LINE_AA)
        cv2.circle(sprite, (cx, cy), self.marker_radius + 3, (255, 255, 255, 255), 2, cv2.LINE_AA)
        ...
        cv2.putText(sprite, label, (bx1 + pad, cy - 2), self.font_cv, 0.6, (30, 30, 30, 255), 1, cv2.LINE_AA)

        return sprite, (cx, cy)

    def blit_sprite(self, frame: np.ndarray, sprite_bgra: np.ndarray, anchor: Tuple[int, int], x: int, y: int):
        h, w = frame.shape[:2]
    ...
    def render_popup_box(self, target_frame: np.ndarray, popup_info: Dict) -> np.ndarray:
        ...
        if img_url and os.path.exists(img_url):
            ...
            if pop_img is not None:
                ...
                text_offset = cv2.getTextSize(label_text or "", self.font_cv, 0.6, 1)[0][1] + 15 if MathUtils.is_real_label(label_text) else 0
                total_w, total_h = pw + (border * 2), ph + (border * 2)

                margin = 40
                box_x = int(popup_info["x"]) - total_w - self.marker_radius - 4 if popup_info["x"] > w * 0.6 else int(popup_info["x"]) + self.marker_radius + 4
                box_y = int(popup_info["y"]) + self.marker_radius + 10 if int(popup_info["y"]) - total_h - text_offset - 10 < margin else int(popup_info["y"]) - total_h - text_offset - 10
    ...
    def create_summary_card(self, distance_km: float, duration_seconds: float, card_size=(460, 100)) -> np.ndarray:
        w, h = card_size
        scale = 2
        ...
        self._draw_walking_icon(draw, pad + icon_size // 2, h * scale // 2, icon_size, icon_color)

        text_x = pad + icon_size + 14 * scale
        draw.text((text_x, 22 * scale), "Time", font=font_label, fill=text_color)
        draw.text((text_x, 44 * scale), self._format_duration_short(duration_seconds), font=font_value, fill=text_color)
    ...
    def composite_card_on_frame(self, frame: np.ndarray, card_bgra: np.ndarray, alpha: float, margin: int = 40) -> np.ndarray:
        out = frame.copy()
        h, w = out.shape[:2]
    ...
    def _draw_walking_icon(self, draw: ImageDraw.ImageDraw, cx: int, cy: int, size: int, color: Tuple):
        r = size // 6
    ...
    def _draw_ruler_icon(self, draw: ImageDraw.ImageDraw, cx: int, cy: int, size: int, color: Tuple):
        half = size // 2
...
class RouteAnimator:
    ...
    def render(self, img_path: str, points: List, labels: List, popups: List, 
               res_sequence: Optional[List] = None, summary: Optional[Dict] = None) -> List[str]:
    ...
    def _draw_prioritized_sprites(self, target_frame: np.ndarray, items_to_draw: List[Dict], sprites_dict: Dict):
        """Draws sprites with collision detection, prioritizing intermediate waypoints."""
        drawn_boxes = []
        
        def get_priority(item):
            idx = item.get("index", -1)
            return 1 if (idx == 0 or idx == getattr(self, '_total_points', 0) - 1) else 2
        ...
        for item in sorted(items_to_draw, key=get_priority, reverse=True):
            ...
            if not any(not (box[2] <= db[0] or box[0] >= db[2] or box[3] <= db[1] or box[1] >= db[3]) for db in drawn_boxes):
    ...
    def _render_overview(self, base_img: np.ndarray, points: List, labels: List, popups: List, fps: int) -> str:
        ...
        named = [(int(points[i][0]), int(points[i][1]), labels[i]) for i in range(len(points)) if MathUtils.is_real_label(labels[i])]
        landmark_sprites = {lbl: self.graphics.prebake_landmark_sprite(lbl) for _, _, lbl in named}
        smooth_path = MapFetcher.get_smooth_path(points, num_frames, ease=True)
        
        active_popups = [
            {"x": points[i][0], "y": points[i][1], "data": popups[i], "label": labels[i], "index": i}
            for i in range(len(points)) if popups and popups[i] is not None
        ]
        ...
        if start_stop_popups:
            ...
            self._draw_prioritized_sprites(intro_frame, start_stop_popups, landmark_sprites)
        ...
        for p in smooth_path:
            ...
            for popup in active_popups:
                if not popup["data"]["triggered"]:
                    ...
                    if MathUtils.point_to_segment_distance(popup["x"], popup["y"], px, py, cx, cy) < trigger_radius:
                        ...
                        self._draw_prioritized_sprites(freeze_frame, trig_popups, landmark_sprites)
    ...
    def _render_waypoints(self, res_sequence: List[Dict], fps: int) -> List[str]:
        ...
        for i, res_data in enumerate(res_sequence):
            ...
            for current_frame in range(total_frames):
                ...
                is_p = any(p["start"] <= current_time_sec <= p["end"] for p in pauses) if pauses else False
...
def main():
    ...
    animator.graphics.line_thickness = args.thickness or settings.get("line_thickness", 10)
