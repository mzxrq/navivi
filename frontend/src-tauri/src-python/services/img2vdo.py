...
class AttractionVideoGenerator:
    ...
    def __init__(self, job_config: Optional[JobConfigManager] = None, workflow_config_path: str = "assets/config/img2vdo-api.json"):
    ...
    def _generate_single_clip(self, local_image_path: str, prompt_text: str) -> Optional[str]:
        ...
        if "error" in res:
            print(f"ComfyUI Error: {res['error']}")
            ws.close()
            return None
        ...
        for node_id in outputs:
            for media in outputs[node_id].get("videos", []):
                ...
                params = {"filename": media["filename"], "subfolder": media.get("subfolder", ""), "type": "output"}
                resp = requests.get(view_url, params=params)
                
                save_path = self.output_dir / f"raw_{uuid.uuid4().hex[:6]}_{media['filename']}"
    ...
    def process_attraction_video(
        ...
        if not popup_image_entry:
            print("No popup image provided for waypoint.")
            return None

        # --- Check list vs string ---
        if isinstance(popup_image_entry, list):
            image_list = [img for img in popup_image_entry if img and os.path.exists(img)]
        elif isinstance(popup_image_entry, str) and os.path.exists(popup_image_entry):
            image_list = [popup_image_entry]
        else:
            print(f"Invalid image entry: {popup_image_entry}")
            return None

        if not image_list:
            return None

        print(f"🎬 Processing waypoint with {len(image_list)} image(s)...")

        # 1. Generate video clips for each image
        generated_clips = []
        for idx, img_path in enumerate(image_list):
            print(f"   -> Rendering image {idx + 1}/{len(image_list)}: {img_path}")
        ...
        if len(generated_clips) > 1:
            print(f"🔄 Combining {len(generated_clips)} clips into a sequence using VideoEditor...")
            ...
            combined_video = self.editor.concatenate_videos(
                ...
                output_filename=temp_combined_name
            )
        else:
        ...
        if target_audio_duration > 0:
            print(f"⏱️ Adjusting video duration to match audio length ({target_audio_duration:.2f}s)...")
        ...
        if audio_path and os.path.exists(audio_path):
            print("🎵 Muxing narration audio into waypoint video...")
        ...
        print(f"✅ Waypoint video deliverable complete: {final_output}")
        return final_output