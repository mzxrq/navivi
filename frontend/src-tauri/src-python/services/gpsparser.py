...
# ... (Keep your GPSDataCleaner class here exactly as it is) ...
...
class GPSJsonExporter:
    ...
    @staticmethod
    def export(cleaned_data: Dict[str, Any], original_input_path: str, project_name: str = "Untitled Project", save_json: bool = True, job_config=None) -> Dict[str, Any]:
        ...
        if save_json:
            # ---------------------------------------------------------------------
            # DYNAMIC PATH FIX: Also route the JSON to the JobConfigManager directory
            # ---------------------------------------------------------------------
            config = job_config or JobConfigManager()
            base_path = Path(config.get("directory_path", "assets"))
            json_dir = (base_path / "json").resolve()
