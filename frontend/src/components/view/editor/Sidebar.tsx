...
export function Sidebar() {
  ...
  // 1. If we are editing, hand off to the Editor component entirely
  if (editingId) {
    return <WaypointEditor wpId={editingId} onClose={() => setEditingId(null)} />;
  }

  // 2. Otherwise, render the List UI
  return (
    <aside ref={sidebarRef} className="w-[340px] bg-white dark:bg-zinc-950 border-r border-zinc-200 dark:border-white/[0.08] flex flex-col p-6 h-full select-none z-10 relative shadow-xl transition-colors gap-6">
      <div className="flex-1 flex flex-col min-h-0 space-y-3">
        <div className="flex justify-between items-center px-1 shrink-0">
          ...
          {waypoints.length > 0 && (
            <button
              onClick={() => { setIsListEditMode(!isListEditMode); setShowClearConfirm(false); }}
          ...
          <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-2">
            ...
            {isListEditMode && (
              <div className="relative mb-3 shrink-0">
                {!showClearConfirm ? (
                  <button onClick={() => setShowClearConfirm(true)} className="w-full py-2.5 rounded-xl border border-red-200 dark:border-red-500/20 bg-red-50 dark:bg-red-500/5 text-red-600 dark:text-red-400 text-xs font-bold hover:bg-red-100 dark:hover:bg-red-500/10 transition-colors flex items-center justify-center gap-2">
                  ...
                  <div className="w-full py-2 px-3 rounded-xl border border-red-300 dark:border-red-500/40 bg-white dark:bg-zinc-900 shadow-sm flex items-center justify-between animate-in fade-in zoom-in-95 duration-200">
                    <span className="text-[11px] font-semibold text-zinc-800 dark:text-zinc-300">Are you sure?</span>
                    <div className="flex items-center gap-2">
                      <button onClick={() => setShowClearConfirm(false)} className="text-[10px] font-medium text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-300 px-2 py-1">Cancel</button>
                      <button onClick={() => { setWaypoints([]); setIsListEditMode(false); setShowClearConfirm(false); }} className="text-[10px] font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10 px-2.5 py-1 rounded-md hover:bg-red-100 dark:hover:bg-red-500/20 transition-colors">Confirm</button>
                    </div>
                  </div>
                )}
              </div>
            ...
            {/* Map over the new WaypointItem component */}
            {waypoints.map((wp, i) => (
              <WaypointItem
                key={wp.id}
                wp={wp}
                ...
                onDelete={() => setWaypoints(waypoints.filter((w) => w.id !== wp.id))}
                onMoveUp={() => moveWaypointUp(i)}
                onMoveDown={() => moveWaypointDown(i)}
              />
            ))}
